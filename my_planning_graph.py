...
class ActionLayer(BaseActionLayer):
    ...
    def _inconsistent_effects(self, actionA, actionB):
        ...
        for e in self.children[actionA]:
            if ~e in self.children[actionB]:
                return True;
            
            
        for e in self.children[actionB]:
            if ~e in self.children[actionA]:
                return True;
    ...
    def _interference(self, actionA, actionB):
        ...
        layers.ActionNode
        """
        for e in actionA.effects:
            if ~e in actionB.preconditions:
                return True;
            
        for e in actionB.effects:
            if ~e in actionA.preconditions:
                return True;     
    ...
    def _competing_needs(self, actionA, actionB):
        ...
        for p1 in actionA.preconditions:
            for p2 in actionB.preconditions:
                check_is_mutex = self.parent_layer.is_mutex (p1,p2);
                if (check_is_mutex):
                    return True;
    ...
    def _inconsistent_support(self, literalA, literalB):
        ...
        for a1 in self.parents[literalA]:
            for a2 in self.parents[literalB]:
                check_is_mutex = self.parent_layer.is_mutex (a1,a2);
                if not check_is_mutex:
                    return False;
        return True
...
class PlanningGraph:
    ...
    def h_levelsum(self):
        ...
        for goal in self.goal:
            costs.append(PlanningGraph.LevelCost(self,goal));
    ...
    def h_setlevel(self):
        ...
        while not self._is_leveled:
            ...
            if AllGoal(last_layer) and NoMutex(last_layer):
                return level
            self._extend()
            level += 1
